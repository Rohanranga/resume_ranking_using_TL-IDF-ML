...
# Calculate suitability percentage based on proximity score and total score
def calculate_suitability_percentage(proximity_score, total_score):
    if proximity_score > 100:
        return 100 # 97% if proximity score exceeds 100
    elif total_score >= 75:
        return 90  # High score category
    elif total_score >= 50:
        return 70  # Medium score category
    else:
        return 30  # Low score category
...
def process_resumes(folder_path, keyword):
    ...
    for filename in os.listdir(folder_path):
        ...
        if text:
            ...
            # Calculate suitability percentage based on proximity and total score
            suitability_percentage = calculate_suitability_percentage(proximity_score, total_score)
            ...
            data.append({
                ...
                "Suitability %": f"{suitability_percentage}%"  # New column
            })
...
def highlight_top_n_rows(n):
    for row in table.get_children():
        table.item(row, tags=())  # Clear previous highlights

    top_n_rows = table.get_children()[:n]
    for row in top_n_rows:
        table.item(row, tags=("highlighted",))
        table.tag_configure("highlighted", background="yellow")  # Highlight color
...
def select_folder():
    ...
    for entry in results:
        table.insert("", "end", values=(
            ...
            entry["Suitability %"]  # New column in table
        ))
    ...
    # Highlight top n rows based on the number of openings
    try:
        n = int(openings_entry.get())
        if n > 0:
            highlight_top_n_rows(n)
    except ValueError:
...
# No. of Openings Input
tk.Label(frame, text="No. of Openings:", font=("Arial", 12)).pack(side="left", padx=10)
openings_entry = tk.Entry(frame, font=("Arial", 12), width=5)
openings_entry.pack(side="left")

columns = ("Rank", "File Name", "Phone Numbers", "Emails", "LinkedIn", "GitHub", "Keyword Count", "Proximity Score", "Total Score", "Experience", "Suitability %")
