...
# Select folder and display results with highlighting
def select_folder():
    ...
    # Highlight top N rows based on "No. of Openings"
    try:
        openings = int(openings_entry.get() or 0)
        if openings > 0:
            # Get all items in the table
            all_items = table.get_children()
            # Highlight the top N rows
            for i, item in enumerate(all_items[:min(openings, len(all_items))]):
                table.item(item, tags=("highlighted",))
    except ValueError:
        messagebox.showwarning("Invalid Input", "Please enter a valid number for 'No. of Openings'.")

    # Configure the highlight tag
    table.tag_configure("highlighted", background="lightgreen")
